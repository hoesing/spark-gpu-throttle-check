...
Usage: python3 spark-gpu-throttle-check.py [--samples N] [--threshold MHZ] [--warmup SECONDS]
"""
...
def fmt(val, spec: str, suffix: str = "") -> str:
...
def gpu_load(stop_event: threading.Event):
...
def run_test(num_samples: int, threshold_mhz: float, warmup: float):
    print("=" * 60)
    ...
    load_thread = threading.Thread(target=gpu_load, args=(stop_event,), daemon=True)
...
def main():
    ...
    sys.exit(run_test(args.samples, args.threshold, args.warmup))
